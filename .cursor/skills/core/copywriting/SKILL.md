...
name: copywriting
description: >-
  User-facing copy rules — no connective dashes between clauses, human error
  messages (what happened, then what to do next), where strings live (the
  ERROR_CODES registry + the ok()/error() envelope). Load when writing or
  reviewing any user-visible text: errors, labels, buttons, toasts, empty
  states, placeholders, microcopy.
---
