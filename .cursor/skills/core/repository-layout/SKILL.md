...
description: >-
  Monorepo layout: apps vs packages, layer-based grouping, {domain}.{sub}.{role} file naming,
  split-on-AND, decoupled OOP services that graduate into packages (app-factory), own-your-interface
  deps, per-package READMEs, no app barrels. Load when adding files, folders, or organizing code.
---
