...
name: try-catch
description: >-
  Prefer tryCatch over try/catch for fallible I/O: failure model (return success / throw error),
  tuple semantics, await rules, layer usage in services, facades, data modules. Load for error handling,
  throws, DB, network, parsing.
---
...
Sync return values are handled immediately; thenables are chained with `Promise.resolve(result)`.
...
`tryCatch` already handles thenables. An `async` callback forces an extra Promise layer and can confuse inference.
...
// ✅ sync callback returning the thenable (or Promise)
const [user, userError] = await tryCatch(() =>
  db.select().from(users).where(eq(users.id, id)).get(),
)
```
