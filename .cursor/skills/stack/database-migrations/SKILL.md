...
description: >-
  D1 migration safety for the deploy pipeline: Phase 3 mutates production before
  code promotion; safe vs destructive DDL; expand-and-contract. Load when authoring
  or reviewing migration SQL, schema changes, drizzle-kit generate, or deploy
  migration steps; dated BACKWARD_COMPAT markers and cleanup prompts. STOP and
  ask the human to choose backward-compatible vs destructive before implementing
  any schema change.
---
...
Expand/contract (and any phased backend change) often leaves **code** that only exists for old deployments still in production. That code must stay **active** until cleanup — do not disable it by commenting it out unless the human explicitly asked to remove the path.

**When you add or leave such a path**, put a **dated marker comment** on the block (or the first line of the island):
...
- Token: `BACKWARD_COMPAT` (grep-friendly). Date: ISO `YYYY-MM-DD` — the day the compat was introduced (usually today).
...
1. Read the marker date; use `git log` / `git blame` on that line to see when it landed and how long ago (~days/weeks).
2. **Stop and ask the human** (do not remove unilaterally), e.g.:

   > This was left for backward compatibility on {date} (~{age} ago). Is the new shape already live in **production**, or only in **staging**? If production has not needed this path for a while, do you want to remove it now (contract / cleanup deploy)?
...
`migrations_dir` and `database_name` live in the API app's deploy config.
