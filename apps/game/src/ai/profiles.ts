 ...
 */
export type BotProfile = ScoringWeights & {
  /** How many plies it looks ahead. */
  depth: number
  /** Chance of ignoring the search and playing at random. */
  caprice: number
}
...
] as const satisfies readonly (keyof BotProfile)[]
 ...
 * others apply flat.
 */
// biome-ignore format: one bot per line, so the eight read as a table and a
// trait can be followed down the column it lives in.
const PROFILES: Record<number, BotProfile> = {
...
export function profileFor(level: number): BotProfile {
