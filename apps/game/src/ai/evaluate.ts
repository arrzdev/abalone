...
/**
 * What the evaluator asks of a caller: one multiplier per term. A bot profile
 * is these plus how far it looks ahead and how often it ignores the answer.
 */
...
for (let from = 0; from < CELL_COUNT; from++) {
  for (let to = 0; to < CELL_COUNT; to++) {
    ...
    PRESSURE[slot] =
      cellRing[to] >= 3 && apart <= 2 ? (apart === 1 ? 3 : 1) : 0
  }
}
...
function centrality(marbles: CellId[]): number {
  let total = 0
...
function pressure(mine: CellId[], theirs: CellId[]): number {
  let total = 0
...
/** How well `side` is queued up behind `theirs` to shove them off. */
function shovePotential(
  theirs: CellId[],
  occupant: Uint8Array,
  side: Side,
): number {
...
function chase(mine: CellId[], theirs: CellId[]): number {
...
function advance(marbles: CellId[], side: Side): number {
...
function exposure(marbles: CellId[]): number {
  let total = 0
...
function loners(marbles: CellId[]): number {
  let alone = 0
 ...
 */
function attackAppetite(ourCentre: number, theirCentre: number): number {
 ...
 *
 * @param sideToMove whose turn it is — decides which side's centre lead the
 *        attacking appetite is measured from
 * @param profile the weights to read the board through, from `profiles.ts`
 */
export function scorePosition(
  ...
  let total =
    white.length === SURVIVORS_AT_DEFEAT
      ? 100
      : black.length === SURVIVORS_AT_DEFEAT
        ? -100
        : 0
  ...
  if (centreWeight !== 0) {
    total += (blackCentre - whiteCentre) * centreWeight
  }
  if (profile.cohesion !== 0) {
    total += (cohesion(black) - cohesion(white)) * profile.cohesion
  }
  if (pressureWeight !== 0) {
    total +=
      (pressure(black, white) - pressure(white, black)) * pressureWeight
  }
  if (shoveWeight !== 0) {
    total +=
      (shovePotential(white, position.occupant, BLACK) -
        shovePotential(black, position.occupant, WHITE)) *
      shoveWeight
  }
  if (profile.chase !== 0) {
    total += (chase(black, white) - chase(white, black)) * profile.chase
  }
  if (profile.charge !== 0) {
    total +=
      (advance(black, BLACK) - advance(white, WHITE)) * profile.charge
  }
  if (profile.recklessness !== 0) {
    total += (exposure(black) - exposure(white)) * profile.recklessness
  }
  if (profile.loner !== 0) {
    total += (loners(black) - loners(white)) * profile.loner
  }
