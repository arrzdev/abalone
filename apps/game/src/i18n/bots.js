...
export const BOT_LEVELS = Object.keys(BOTS).map(Number);
...
export const blurbKey = (level) => `bots:${getBot(level).id}.blurb`;
export const titleKey = (level) => `bots:${getBot(level).id}.title`;
 ...
 */
export function pickLine(level, event, avoid = null) {
  const { id } = getBot(level);
  const pool = enBots[id]?.[event];
  if (!pool?.length) return null;

  const keys = pool.map((_, i) => `bots:${id}.${event}.${i}`);
  const choices = keys.length > 1 ? keys.filter((key) => key !== avoid) : keys;
