...
/** What every marble design is: one ball, painted where it is told. */
...
function strokeMarbleOutline(
  ...
  isHovered: boolean,
  baseLineWidth: number,
  defaultStroke: { color: string; widthFactor: number },
): void {
  ctx.beginPath()
...
const renderDefaultMarble: MarbleRenderer = (
  ctx,
  x,
  y,
  radius,
 ...
 */
const BODIES: Record<"white" | "black", Body> = {
  white: {
    key: "ivory",
    base: [252, 251, 249],
    sky: 0.38,
...
function shadeBall(
  ...
  for (let py = 0; py < side; py++) {
    ...
    for (let px = 0; px < side; px++) {
      ...
      const spec = ndh > 0 ? ndh ** body.shine : 0
 ...
 */
function shadowSprite(side: number): HTMLCanvasElement {
...
function bitmapFor(
  key: string,
  side: number,
  build: (side: number) => HTMLCanvasElement,
): HTMLCanvasElement {
  const id = `${key}|${side}`
...
function ballFor(body: Body, side: number): HTMLCanvasElement {
  return bitmapFor(body.key, side, (px) => {
    const canvas = document.createElement("canvas")
    canvas.width = px
    canvas.height = px
    const ctx = canvas.getContext("2d")
    if (ctx) ctx.putImageData(shadeBall(ctx, px, body), 0, 0)
    return canvas
  })
}

/** Whole device pixels, so a blit is a copy rather than a resample. */
const snapped = (v: number, scale: number) => Math.round(v * scale) / scale

const render3DMarble: MarbleRenderer = (
  ctx,
  x,
  y,
  radius,
  ...
  const body =
    color === "#fff" || color === "white" ? BODIES.white : BODIES.black
  ...
  strokeMarbleOutline(
    ...
    isHovered,
    baseLineWidth,
    { color: body.outline, widthFactor: 0.9 },
  )
}
...
export const MARBLE_DESIGNS = {
  ...
  "3d": render3DMarble,
} as const satisfies Record<string, MarbleRenderer>

export type MarbleDesign = keyof typeof MARBLE_DESIGNS

/** Which designs are lit, and so have something to cast a shadow with. */
const LIT = new Set<string>(["3d"])

export function hasDesign(designName: string): designName is MarbleDesign {
...
export function renderMarble(
  ...
  const render = hasDesign(designName)
    ? MARBLE_DESIGNS[designName]
    : MARBLE_DESIGNS.default
 ...
 */
export function renderMarbleShadow(
  designName: string,
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  ...
  if (!LIT.has(designName) || typeof document === "undefined") return
  if (!(radius > 0)) return
  const scale = scaleOf(ctx)
  ...
  ctx.drawImage(
    bitmapFor("shadow", side, shadowSprite),
    cx - reach,
    cy - reach,
    reach * 2,
    reach * 2,
