...
const rankOf = (event: SayEvent | null) =>
  event === null ? -1 : EVENT_RANK.indexOf(event)

export type GamePhase = "pregame" | "ingame" | "postgame"

export type BotChatterOptions = {
  level: number
  enabled: boolean
  phase: GamePhase
  winner: Player | null
...
export function useBotChatter({
  ...
  const lastEvent = useRef<SayEvent | null>(null)
  ...
  const wasAtRisk = useRef<Record<Player, Set<CellName>>>({
    black: new Set(),
    white: new Set(),
  })
  ...
  useEffect(() => {
    ...
    const playerColor: Player = botColor === "black" ? "white" : "black"
    const atRisk = {
      [botColor]: marblesAtRisk(state, botColor),
      [playerColor]: marblesAtRisk(state, playerColor),
    } as Record<Player, Set<CellName>>
    ...
    const arose = (color: Player) =>
      [...atRisk[color]].some(
        (marble) => !wasAtRisk.current[color].has(marble),
      )

    const offered = new Set<SayEvent>()
    if (details.isCapture) {
      offered.add(
        details.color === botColor
    ...
    const chosen =
      event ??
      (moveIndex - quietSince.current >= AMBIENT_GAP ? SAY.AMBIENT : null)
    if (!chosen) return
