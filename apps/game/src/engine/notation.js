import { CELL_COUNT, NOWHERE, cellCol, cellNamed, cellRow, separation } from './topology.js';
...
const LABELS = new Array(CELL_COUNT);
for (let cell = 0; cell < CELL_COUNT; cell++) {
  LABELS[cell] = `${String.fromCharCode(97 + 4 - cellRow[cell])}${cellCol[cell] + 5}`;
}

/** The algebraic label for a `"r,q"` square, or '' if it names none. */
export function squareLabel(name) {
 ...
 * @param {string[]} marbles the line that moved, in any order
 * @param {string} destination the square it moved onto
 */
export function formatMoveAlgebraic(marbles, destination) {
