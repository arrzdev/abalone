...
import { formatMoveAlgebraic } from '../engine/notation.js';
...
function MoveContent({ details, marbleDesign }) {
  const { t } = useTranslation();
  const notation = formatMoveAlgebraic(details.marbles, details.destination);
  ...
  return (
    <span className="flex items-center gap-1.5">
      <span className="flex gap-px" title={t('game:history.marbles_moved', { n: count })}>
        {Array.from({ length: count }, (_, i) => (
          <span key={i} className="h-1.5 w-1.5 rounded-full bg-current opacity-50" />
        ))}
      </span>
      <span className="font-mono">{notation}</span>
      {details.isPush && (
        <span className="text-brand-light" title={t('game:history.pushed')}>▸</span>
      )}
 ...
 */
function StripContent({ details, marbleDesign }) {
  const { t } = useTranslation();
  const notation = formatMoveAlgebraic(details.marbles, details.destination);
  if (!notation) return t('game:history.move');
 ...
 */
export function MoveStrip({ moveHistory, currentMoveIndex, onGoTo, marbleDesign, result, className }) {
  const { t } = useTranslation();
  ...
  if (moves.length === 0 && !result) {
    return (
      <div className={cn('flex items-center justify-center text-sm text-white/30', className)}>
  ...
  return (
    <div className={cn('overflow-hidden', className)}>
      <div ref={stripRef} className="strip-scroll flex h-full items-center gap-1 overflow-x-auto">
        ...
        <TapButton
          ...
          className={cn(
            ...
            currentMoveIndex === 0 ? 'bg-brand text-white' : 'text-white/50 hover:bg-white/5 hover:text-white',
          )}
        >
          ...
          <Fragment key={index}>
            ...
            >
              {entry.moveDetails?.marbles ? (
                <StripContent details={entry.moveDetails} marbleDesign={marbleDesign} />
              ) : (
                t('game:history.move')
              )}
            </TapButton>
 ...
 * exist.
 */
function ResultLine({ result, marbleDesign, className }) {
  return (
    <div className={cn('flex items-center justify-center gap-1.5 font-semibold text-white/70', className)}>
      {result.winner && <MarbleGlyph color={result.winner} design={marbleDesign} size={11} />}
      {result.label}
    </div>
  );
}
 ...
 */
function StartRow({ current, onGoTo }) {
  const { t } = useTranslation();
  return (
    ...
    <div className="border-b border-white/5 px-1 py-0.5">
      <TapButton
        ...
        className={cn(
          ...
          'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand',
          current ? 'bg-brand text-white' : 'text-white/50 hover:bg-white/5 hover:text-white',
        )}
      >
        {t('game:history.start')}
      </TapButton>
...
function MoveCell({ entry, index, currentIndex, onGoTo, marbleDesign }) {
  const { t } = useTranslation();
  ...
  return (
    <TapButton
      ...
      className={cn(
        ...
        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand',
        isCurrent ? 'bg-brand text-white' : 'text-white/70 hover:bg-white/5 hover:text-white',
      )}
    >
      {entry.moveDetails?.marbles ? (
        <MoveContent details={entry.moveDetails} marbleDesign={marbleDesign} />
      ) : (
        t('game:history.move')
      )}
    </TapButton>
 ...
 */
export function MoveHistory({ moveHistory, currentMoveIndex, onGoTo, marbleDesign, result, className }) {
  const { t } = useTranslation();
    ...
    <div className={cn('relative overflow-hidden', className)}>
      <div ref={listRef} className="panel-scroll h-full overflow-y-auto">
        ...
        {pairs.map((pair) => (
          <div
            key={pair.blackIndex}
            data-current={pair.blackIndex === currentMoveIndex || pair.whiteIndex === currentMoveIndex}
            className="flex items-center gap-1 border-b border-white/5 px-1 py-0.5 last:border-b-0"
          >
            <span className="w-7 shrink-0 text-right text-xs text-white/30 tabular-nums">{pair.moveNumber}.</span>
            <MoveCell
              entry={pair.black}
        ...
        {result && <ResultLine result={result} marbleDesign={marbleDesign} className="px-1 py-2 text-sm" />}
      </div>
      ...
      <EdgeFade className="bottom-0 bg-linear-to-t" visible={edges.bottom} />
    </div>
  );
}
 ...
 */
function EdgeFade({ visible, className }) {
  return (
    <div
      aria-hidden="true"
      className={cn(
