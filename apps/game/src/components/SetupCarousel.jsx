...
import { getSetupName } from '../i18n/gameText.js';
 ...
 */
export function SetupCarousel({ setupType, onChange }) {
  const { t } = useTranslation();
  const index = Math.max(0, SETUP_ORDER.indexOf(setupType));

  const step = useCallback(
    (delta) => {
      const next = (index + delta + SETUP_ORDER.length) % SETUP_ORDER.length;
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 rounded-xl bg-surface-4 p-1">
        ...
        <span className="flex-1 truncate text-center font-semibold text-white">{getSetupName(setupType)}</span>
