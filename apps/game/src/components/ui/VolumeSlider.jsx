...
import { VolumeOffIcon, VolumeUpIcon } from '../Icons.jsx';
      ...
      <div className="flex items-center gap-3">
        <TapButton
          ...
          className={cn(
            ...
            muted ? 'bg-elevated-2 text-white' : 'bg-surface-4 text-white/70 hover:text-white',
          )}
        >
          {muted ? <VolumeOffIcon size={18} /> : <VolumeUpIcon size={18} />}
        </TapButton>
        ...
        <input
          ...
          onKeyUp={(event) => { if (MOVES_IT.has(event.key)) playMoveSound(1); }}
          aria-label={label}
