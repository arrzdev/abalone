import { cellNamed, nameOf, NOWHERE } from '../engine/topology.js';
import { BLACK, createPosition, rival, sideFromName, signature } from '../engine/position.js';
import { commitMove, destinationsFor, linesFor, resolveMove } from '../engine/moves.js';
import { scorePosition } from './evaluate.js';
...
/**
 * @param {number} level 1..8, see `profiles.js`
 * @returns {{chooseMove: (board: {black: string[], white: string[], turn: string}) => object}}
 */
export function createBot(level) {
  ...
  function step(position, line, target, side) {
    const plan = resolveMove(position, line, target, side);
    return commitMove(position, line, plan.heading, plan.shoved, side);
  }

  /** What this position is worth to black, standing still. */
  function assess(position, side, stamp) {
  ...
  function explore(position, side, depth, alpha, beta, stamp) {
    ...
    for (const line of lines) {
      for (const target of destinationsFor(position, line, side)) {
        ...
        const value = explore(after.position, foe, depth - 1, alpha, beta, nextStamp);
  ...
  function chooseMove(board) {
    ...
    for (const line of lines) {
      for (const target of destinationsFor(position, line, side)) {
        ...
        const value = explore(after.position, foe, profile.depth - 1, alpha, beta, stamp);
    ...
    if (bestLine === null) {
      ...
      const fallback = lines.length > 0 ? destinationsFor(position, lines[0], side) : [];
      if (fallback.length === 0) return { type: 'move', selection: null, move: null, score: best };
