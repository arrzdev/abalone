...
import { HEADING_STEPS } from '../engine/topology.js';
import { RuleDiagram } from '../components/RuleDiagram.jsx';
import { MarbleGlyph } from '../components/MarbleGlyph.jsx';
...
import { loadPreference } from '../utils/preferences.js';
...
const PUSH = {
  ...
  marbles: { '0,-3': 'white', '0,-2': 'white', '0,-1': 'white', '0,0': 'black', '0,1': 'black' },
  arrows: [
    { pos: '0,-3', dir: [0, 1] },
    { pos: '0,-2', dir: [0, 1] },
    { pos: '0,-1', dir: [0, 1] },
...
const CAPTURE = {
  cells: ['0,-2', '0,-1', '0,0'],
  marbles: { '0,-2': 'white', '0,-1': 'white', '0,0': 'black', '0,1': 'black' },
  arrows: [
    { pos: '0,-2', dir: [0, 1] },
    { pos: '0,-1', dir: [0, 1] },
    { pos: '0,0', dir: [0, 1] },
...
const COMPASS = {
  ...
  marbles: { '0,0': 'white' },
  arrows: HEADING_STEPS.map((dir) => ({ pos: `${dir[0]},${dir[1]}`, dir })),
};
...
function Figure({ diagram, caption, marbleDesign }) {
  return (
    <figure className="mt-4">
      <div className="overflow-hidden rounded-xl bg-black/20 p-2">
        <RuleDiagram diagram={diagram} marbleDesign={marbleDesign} label={caption} />
      </div>
      <figcaption className="mt-2 text-center text-xs text-white/45">{caption}</figcaption>
    </figure>
  );
}

export function RulesPage({ onBack, onPlay }) {
  const { t } = useTranslation();

  // The diagrams are the player's own marbles, not a stock illustration.
  const stored = loadPreference('marbleDesign', 'default');
  const marbleDesign = hasDesign(stored) ? stored : 'default';
    ...
    <div className="relative h-full overflow-hidden bg-elevated">
      ...
      <div className="relative h-full overflow-y-auto overscroll-contain">
        <div className="mx-auto w-full max-w-2xl px-4 py-6">
          ...
          <p className="mb-6 text-sm leading-relaxed text-white/60">{t('game:rules.intro')}</p>

          <div className="space-y-4">
            <Section title={t('game:rules.goal.title')}>
              <p className="mt-2 text-sm leading-relaxed text-white/70">{t('game:rules.goal.body')}</p>
              ...
              <div className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-black/20 py-4">
                {Array.from({ length: WINNING_SCORE }, (_, i) => (
                  <MarbleGlyph key={i} color="black" design={marbleDesign} size={24} />
                ))}
              </div>
            </Section>

            <Section title={t('game:rules.directions.title')}>
              <p className="mt-2 text-sm leading-relaxed text-white/70">{t('game:rules.directions.body')}</p>
              <Figure
                diagram={COMPASS}
            ...
            <Section title={t('game:rules.move.title')}>
              <p className="mt-2 text-sm leading-relaxed text-white/70">{t('game:rules.move.body')}</p>
              <Figure diagram={INLINE} caption={t('game:rules.move.inline_caption')} marbleDesign={marbleDesign} />
              <Figure
                diagram={BROADSIDE}
            ...
            <Section title={t('game:rules.push.title')}>
              <p className="mt-2 text-sm leading-relaxed text-white/70">{t('game:rules.push.body')}</p>
              <Figure diagram={PUSH} caption={t('game:rules.push.ok_caption')} marbleDesign={marbleDesign} />
              <Figure
                diagram={BLOCKED}
            ...
            <Section title={t('game:rules.capture.title')}>
              <p className="mt-2 text-sm leading-relaxed text-white/70">{t('game:rules.capture.body')}</p>
              <Figure diagram={CAPTURE} caption={t('game:rules.capture.caption')} marbleDesign={marbleDesign} />
            </Section>
          </div>

          <Button variant="primary" size="lg" className="mt-6 w-full" onClick={onPlay}>
            {t('game:rules.cta')}
          </Button>
        </div>
      </div>
