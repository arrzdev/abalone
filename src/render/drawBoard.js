import { MAX_LINE } from '../engine/config.js';
import { NOWHERE, cellNamed } from '../engine/topology.js';
import { isValidSelection } from '../engine/gameState.js';
import { directionBetween } from '../engine/rules.js';
import { STEP, cellFromPoint, hexCenter } from './hexGrid.js';
import { renderMarble, renderMarbleShadow } from './marbleRenderer.js';
...
function drawFieldCircle(ctx, x, y, radius, baseLineWidth) {
  ctx.beginPath();
...
function drawMoveIndicator(ctx, x, y, radius, fillStyle = '#ccc') {
  ctx.beginPath();
...
function drawCoordinateLabels(ctx, view) {
 ...
 */
function leavesTheBoard(names, direction) {
 ...
 *
 * @param {string[]} marbles squares the group is leaving
 * @param {boolean} onDark   the group is the dark side, so the furrow is cut light
 * @param {string|null} target square the move is aimed at, when there is one
 * @param {number[]|null} direction heading, for a group with no target of its own
 * @param {boolean} pending  the move has not been made — the marbles are still at the near end
 * @returns {null|{centres: Array<{x:number,y:number}>, angle: number, onDark: boolean, pending: boolean}}
 */
function furrowFor(view, state, marbles, onDark, target, direction = null, pending = false) {
  ...
  return {
    ...
    key: `${pending ? 'hover' : 'move'}:${marbles.join('|')}>${target ?? direction}`,
 ...
 */
function animatedFurrows(view, state, animation) {
  const flip = state.shouldFlipBoard;
  ...
  const centres = marbles.map((marble) => centreOf(marble.from, view, flip));
  ...
  centres.forEach((centre, i) => {
    if (centre.x * cos + centre.y * sin < centres[rear].x * cos + centres[rear].y * sin) rear = i;
  });
  ...
  // see `furrowsToShow`.
  const overTheRim = marbles.some((marble) => cellNamed(marble.to) === NOWHERE);

  return [{
    key: `move:${marbles.map((marble) => marble.from).join('|')}>${dr},${dq}`,
    centres,
    angle,
 ...
 *
 * @returns {Array<object>} the furrows to paint, each with a `key` that names
 *   the move rather than the pixels, so a caller can tell them apart over time
 */
export function furrowsToShow(view, state, possibleMoves = [], animation = null, isAnimating = false) {
  if (isAnimating) return animatedFurrows(view, state, animation);

  const furrows = [];

  if (state.lastMove) {
    const { fromMarbles, direction, marbles, shovedMarbles } = state.lastMove;
    ...
    const furrow = furrowFor(view, state, shifted, moverIsDark, null, direction);
  ...
  if (!state.gameOver && state.hoveredCell && possibleMoves.includes(state.hoveredCell)) {
    const picked = state.selectedMarbles;
    // Nothing has moved yet, so the marbles are still at the near end.
    furrows.push(furrowFor(view, state, picked, state.black.has(picked[0]), state.hoveredCell, null, true));
  }

  return furrows.filter(Boolean);
}
 ...
 */
function linesOfTravel(centres, cos, sin, step) {
  const lines = [];
  for (const centre of centres) {
    ...
    const line = lines.find((known) => Math.abs(known.across - across) < step * 0.3);
    if (line) {
 ...
 */
function paintFurrow(ctx, { centres, angle, onDark, pending, reach, fade = 1 }, radius, spacing) {
  if (fade <= 0) return;
  const cos = Math.cos(angle);
  const sin = Math.sin(angle);
  ...
  ctx.save();
  for (const { across, from, to } of linesOfTravel(centres, cos, sin, step)) {
    ...
    ctx.beginPath();
    ctx.arc(dome.x, dome.y, half, angle + (sweep * Math.PI) / 2, angle - (sweep * Math.PI) / 2);
    ctx.lineTo(cut.x - sweep * half * acrossX, cut.y - sweep * half * acrossY);
    ctx.lineTo(cut.x + sweep * half * acrossX, cut.y + sweep * half * acrossY);
    ctx.closePath();
 ...
 * @param {{centerX:number,centerY:number,radius:number,spacing:number,baseLineWidth:number,width:number,height:number}} options.view
 * @param {object} options.state       game state
 * @param {string[]} options.possibleMoves
 * @param {string} options.marbleDesign
 * @param {boolean} options.showCoordinates
 * @param {boolean} options.reviewing   painting an earlier position, not the live game
 * @param {null|{positions: Map<string,{x:number,y:number}>, movingMarbles: Array, direction: number[]}} options.animation
 */
export function drawBoard(ctx, { view, state, possibleMoves = [], marbleDesign = 'default', showCoordinates = false, reviewing = false, animation = null, fading = [] }) {
  const { centerX, centerY, radius, spacing, baseLineWidth, width, height } = view;
  const flip = state.shouldFlipBoard;
  ...
  drawHexagon(ctx, centerX, centerY, radius * 10 - baseLineWidth * 2, reviewing ? COLORS.reviewing : COLORS.board);
  ctx.stroke();
  ...
  for (const furrow of furrowsToShow(view, state, possibleMoves, animation, isAnimating)) {
    paintFurrow(ctx, furrow, radius, spacing);
  }
  ...
  const hoverable =
    !selectionFull && state.hoveredCell && isValidSelection(state, state.hoveredCell)
      ? state.hoveredCell
      : null;
  ...
  forEachCell((r, q) => {
    ...
    const animated = isAnimating ? animation.positions.get(coordKey) : null;
    if (animated) {
      renderMarbleShadow(marbleDesign, ctx, animated.x, animated.y, radius * MARBLE);
      renderMarble(marbleDesign, ctx, animated.x, animated.y, radius * MARBLE, animated.color, isSelected, isHovered, baseLineWidth);
    } else if (state.black.has(coordKey)) {
      renderMarbleShadow(marbleDesign, ctx, pos.x, pos.y, radius * MARBLE);
      renderMarble(marbleDesign, ctx, pos.x, pos.y, radius * MARBLE, '#333', isSelected, isHovered, baseLineWidth);
    } else if (state.white.has(coordKey)) {
      renderMarbleShadow(marbleDesign, ctx, pos.x, pos.y, radius * MARBLE);
      renderMarble(marbleDesign, ctx, pos.x, pos.y, radius * MARBLE, '#fff', isSelected, isHovered, baseLineWidth);
    }

    if (!state.gameOver && !isAnimating && possibleMoves.includes(coordKey)) {
  ...
  if (showCoordinates) {
    forEachCell((r, q) => {
      const pos = hexCenter(r, q, centerX, centerY, spacing, flip);
      const coordKey = `${r},${q}`;
      const marbleColor = state.black.has(coordKey) ? '#333' : state.white.has(coordKey) ? '#fff' : null;
      ...
      ctx.fillStyle = marbleColor === '#333' ? '#fff' : '#000';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
