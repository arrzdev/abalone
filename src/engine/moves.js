...
import { VACANT, rival } from './position.js';
 ...
 *
 * @returns {number[][]} lines, each running end to end
 */
export function linesFor(position, side) {
...
function loneSteps(position, cell) {
  const found = [];
  for (let heading = 0; heading < HEADINGS; heading++) {
 ...
 */
function collectInline(found, position, line, tip, heading, side) {
  ...
  if (cursor === NOWHERE || position.occupant[cursor] === VACANT) found.push(target);
}
 ...
 */
function collectBroadside(found, position, line, tip, facing) {
 ...
 *
 * @returns {number[]} destination cells, in-line first and broadside after
 */
export function destinationsFor(position, line, side) {
 ...
 *
 * @returns {null|{anchor: number, heading: number, shoved: number[]}}
 *          `shoved` runs from the marble the line touches outward.
 */
export function resolveMove(position, line, target, side) {
  let anchor = NOWHERE;
  let heading = NOWHERE;
  for (const cell of line) {
 ...
 *
 * @returns {null|{position: object, landings: number[], shovedTo: number[], captured: number}}
 *          `shovedTo` holds NOWHERE for each marble driven over the rim.
 */
export function commitMove(position, line, heading, shoved, side) {
  const landings = new Array(line.length);
  for (let i = 0; i < line.length; i++) {
    landings[i] = neighbour(line[i], heading);
    if (landings[i] === NOWHERE) return null; // a side never walks off its own board
  }

  const shovedTo = new Array(shoved.length);
  for (let i = 0; i < shoved.length; i++) shovedTo[i] = neighbour(shoved[i], heading);
  ...
  const ownRoster = position.roster[side].filter((cell) => !line.includes(cell));
