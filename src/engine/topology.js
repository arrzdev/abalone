 ...
 * (see `rules.js`); nothing inside the engine handles a string.
 */
 ...
 */
export const HEADING_STEPS = Object.freeze([
  [1, 0],
  [1, -1],
  [0, -1],
  [-1, 0],
...
export const reverse = (heading) => (heading + 3) % HEADINGS;

/** The heading `turns` sixths-of-a-turn round from this one; `turns` may be negative. */
export const veer = (heading, turns) => (heading + turns + HEADINGS) % HEADINGS;
...
export const cellNames = new Array(CELL_COUNT);
const idsByName = new Map();

const ADJACENCY = new Int8Array(CELL_COUNT * HEADINGS);
const HEADING_BETWEEN = new Int8Array(CELL_COUNT * CELL_COUNT).fill(NOWHERE);
 ...
 */
function axialDistance(r1, q1, r2, q2) {
  return Math.max(Math.abs(r1 - r2), Math.abs(q1 - q2), Math.abs(r1 + q1 - r2 - q2));
}

/** The board is every square within RIM steps of the centre. That is all it is. */
function inBounds(r, q) {
...
for (let id = 0; id < CELL_COUNT; id++) {
  for (let heading = 0; heading < HEADINGS; heading++) {
    ...
    ADJACENCY[id * HEADINGS + heading] = cellAt(cellRow[id] + dr, cellCol[id] + dq);
  }
}
...
export const neighbour = (cell, heading) => ADJACENCY[cell * HEADINGS + heading];

/** The heading that takes you from `from` to `to` in a single step, else NOWHERE. */
export const headingBetween = (from, to) => HEADING_BETWEEN[from * CELL_COUNT + to];

/** Hex distance between two cells. */
export const separation = (from, to) => SEPARATION[from * CELL_COUNT + to];

/** `"r,q"` for a cell id. */
export const nameOf = (cell) => cellNames[cell];
...
export const ALL_CELLS = Object.freeze([...Array(CELL_COUNT).keys()]);
