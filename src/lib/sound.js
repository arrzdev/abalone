...
let context = null;
let master = null;
const buffers = new Map(); // url -> AudioBuffer, or a Promise while it loads
...
export function primeSounds() {
  ...
  Object.values(CLIPS).forEach(bufferFor);
}

/**
 * Plays one clip.
 *
 * @param {'move'|'fall'} name
 * @param {number} [gain] multiplier on top of the user's volume; the marble
 *        ladder lives above unity, so this is not clamped to 1.
 */
export function playSound(name, gain = 1) {
  ...
  Promise.resolve(bufferFor(url)).then((buffer) => {
    ...
    if (!buffer || muted || volume <= 0) return;
...
export function playMoveSound(marbleCount) {
  const step = Math.min(Math.max(Math.round(marbleCount) || 1, 1), WEIGHT.length - 1);
...
export function setSoundVolume(next) {
  ...
  volume = Number.isFinite(next) ? Math.min(Math.max(next, 0), 1) : DEFAULT_VOLUME;
