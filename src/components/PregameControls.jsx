...
import { SetupCarousel } from './SetupCarousel.jsx';
import { Button } from './ui/Button.jsx';
import { SegmentedControl } from './ui/SegmentedControl.jsx';
import { GroupIcon, RobotIcon } from './Icons.jsx';
import { TapButton } from './ui/TapButton.jsx';
import { avatarSrc, blurbKey, titleKey } from '../i18n/bots.js';
import { BOT_LEVELS, getBotName } from '../i18n/gameText.js';
import { cn } from '../lib/cn.js';

/** Black first, because black moves first. */
const LOCAL_SIDES = ['black', 'white'];

function Section({ title, children }) {
  return (
    <section>
      <h2 className="mb-2 text-xs font-semibold tracking-wider text-white/40 uppercase">{title}</h2>
      {children}
    </section>
  );
}
  ...
  return (
    <div className="panel-scroll flex-1 space-y-6 overflow-y-auto px-4 py-4">
      <SegmentedControl
        ...
        options={[
          { value: 'ai', label: t('game:controls.mode_ai'), icon: <RobotIcon size={18} /> },
          { value: 'local', label: t('game:controls.mode_local'), icon: <GroupIcon size={18} /> },
        ]}
      />
      ...
      <Section title={t('game:controls.board_setup')}>
        ...
        {preview && (
          <div className="mt-3 flex aspect-[8/7] items-center justify-center lg:hidden">{preview}</div>
        )}
      </Section>
      ...
      {isLocal ? (
        <>
          <div className="rounded-xl bg-surface-4 p-4">
            <h3 className="font-bold text-white">{t('game:local.title')}</h3>
            <p className="mt-1 text-sm leading-relaxed text-white/60">{t('game:local.description')}</p>
          </div>
          ...
          <Section title={t('game:local.names')}>
            ...
            <div className="space-y-2">
              {LOCAL_SIDES.map((color) => (
                <div key={color} className="flex items-center gap-2.5 rounded-xl bg-surface-4 px-3">
                  <MarbleGlyph color={color} design={marbleDesign} size={14} />
                  <input
                    type="text"
                    value={names[color]}
                    onChange={(event) => onNameChange(color, event.target.value)}
                ...
                return (
                  <TapButton
                    ...
                    className={cn(
                      ...
                      selected ? 'bg-brand' : 'bg-surface-4 hover:bg-surface-5',
                    )}
                  >
                    <span
                      className={cn(
                    ...
                    >
                      <img src={avatarSrc(level)} alt="" className="block h-full w-full" />
