...
import { cellFromPoint, hexCenter } from '../render/hexGrid.js';
import { drawBoard, furrowsToShow } from '../render/drawBoard.js';
import { TIMING, arrivedByPlaying, easeInOutCubic, interpolate } from '../render/motion.js';
import { TAP_SLOP, markTapHandled } from '../hooks/useClickFix.js';
...
export const GameCanvas = forwardRef(function GameCanvas(
  {
    state, possibleMoves, marbleDesign, showCoordinates,
    notice = null, noticeAction = null, onReturnToLatest,
    interactive = true, onCellClick, onDragSelect, onHover, onResize,
  },
  ref,
) {
  ...
  propsRef.current = { state, possibleMoves, marbleDesign, showCoordinates, reviewing: Boolean(notice) };
  ...
  const wasAtRef = useRef({ index: null, moves: 0 });
   ...
   */
  const noteArrival = useCallback((state) => {
    const at = { index: state.currentMoveIndex, moves: state.moveHistory.length };
  ...
  const collectFading = useCallback((live, now) => {
    ...
    for (const gone of shownRef.current) {
      ...
      if (fadingRef.current.some((ghost) => ghost.key === gone.key)) continue;
    ...
    fadingRef.current = fadingRef.current.filter((ghost) => !alive.has(ghost.key));
    ...
    fadingRef.current = fadingRef.current.filter((ghost) => now - ghost.since < TIMING.FURROW_FADE);
    return ghosts;
  }, []);
  ...
  const draw = useCallback(() => {
    ...
    const { state: s, possibleMoves: moves, marbleDesign: design, showCoordinates: coords, reviewing } = propsRef.current;
    const view = viewRef.current;
    const animation = animationRef.current;
    const now = performance.now();
    noteArrival(s);
    const fading = collectFading(furrowsToShow(view, s, moves, animation, Boolean(animation)), now);
    ...
    // the work.
    if (fadingRef.current.length && rafRef.current === null && fadeRafRef.current === null) {
  ...
  const resize = useCallback(() => {
    ...
    const scaleFactor = Math.min(availableWidth / BASE_WIDTH, availableHeight / BASE_HEIGHT);
      ...
      new Promise((resolve) => {
        ...
        const entries = moveData.movingMarbles.map((marble) => {
          ...
          return {
            key: marble.from,
            color: marble.color,
            from: hexCenter(fromR, fromQ, view.centerX, view.centerY, view.spacing, flip),
            to: hexCenter(toR, toQ, view.centerX, view.centerY, view.spacing, flip),
          };
        });
        ...
        const step = (now) => {
          ...
          for (const entry of entries) {
            ...
            positions.set(entry.key, { x: point.x, y: point.y, color: entry.color });
          }
  ...
  useImperativeHandle(ref, () => ({ animateMove, redraw: draw }), [animateMove, draw]);
  ...
  const cursor =
    interactive &&
    hoveredPos &&
    (state.black.has(hoveredPos) || state.white.has(hoveredPos) || possibleMoves.includes(hoveredPos))
      ? 'pointer'
      : 'default';
        ...
        {notice && (
          <div className="absolute inset-x-0 top-[1.01%] flex h-[5.33%] items-center justify-center">
            ...
            <TapButton
              ...
              style={{ fontSize: 'clamp(0.625rem, calc(var(--board-h) * 0.028), 1.125rem)' }}
              className={cn(
