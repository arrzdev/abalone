...
import { LANGUAGE_FLAGS, LANGUAGE_NAMES, SUPPORTED_LANGUAGES, changeLanguage } from '../i18n/index.js';
 ...
 */
export function LanguageSwitcher({ className, variant = 'ghost' }) {
  const { i18n, t } = useTranslation();
  ...
  return (
    <div className={cn('relative', className)} ref={containerRef}>
      ...
      >
        <Flag code={current} />
        <span className="text-sm leading-none font-semibold tracking-wide">{current.toUpperCase()}</span>
      </TapButton>
      ...
      {open && (
        ...
        <div className="absolute right-0 z-50 mt-2 w-44 overflow-hidden rounded-xl bg-surface-2 shadow-2xl shadow-black/50">
          <ul
            role="listbox"
