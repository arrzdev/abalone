...
function Action({ icon, label, onClick, disabled, busy, title }) {
  return (
    <TapButton
      onClick={onClick}
      disabled={disabled}
...
export function IngameControls({
  ...
  const history = { moveHistory: state.moveHistory, currentMoveIndex: state.currentMoveIndex, onGoTo, marbleDesign };
