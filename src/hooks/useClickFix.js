...
export function isTapEcho(event) {
...
/**
 * @param {(event: React.PointerEvent|React.MouseEvent) => void} [callback]
 * @returns {{onPointerDown: Function, onPointerUp: Function, onPointerCancel: Function, onClick: Function}}
 */
export function useClickFix(callback) {
  ...
  const onPointerDown = useCallback((event) => {
    ...
    pressRef.current =
      event.button === 0 && event.isPrimary ? { x: event.clientX, y: event.clientY } : null;
  }, []);
