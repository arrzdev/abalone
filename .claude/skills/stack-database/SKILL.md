---
name: stack-database
description: Drizzle over Cloudflare D1: the cached getDb factory, where queries live, batch versus transaction, and the D1 batch-style transaction trap. Use when writing queries, atomic writes, db.batch, or db.transaction.
---
...
**Errors:** service methods wrap their own fallible work in `tryCatch` and throw `CustomError`. Inside the transaction callback, **await services directly** — their throws roll back the tx. The facade does not `tryCatch` each call unless orchestration needs a corrective branch.
