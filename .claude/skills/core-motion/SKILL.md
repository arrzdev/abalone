---
name: core-motion
description: When to animate and when not to, easing, duration tiers, and press feedback: scale only, and only on tap-to-commit controls. Use when adding a transition, animation, or press state.
---
...
- Prefer **`pressed:`** (JS `data-pressed` from the gesture engine — reentrant, and more reliable than CSS `:active` on iOS; see `platform-ios-webkit`). Use `active:` only on raw elements not wired to the engine.
