...
(Full env model: `stack-env-config`.) Vite string-replaces the **literal** `import.meta.env` at build — with the value for `import.meta.env.VITE_FOO`, or with the whole client env object for `key in import.meta.env` / `import.meta.env[key]`. Alias it first (`const e = import.meta.env; e[key]`) and the replacement never fires: the alias stays a runtime reference, the `VITE_*` registry ships **empty**, and every read is `undefined`. Keep `import.meta.env` inline as a single token everywhere — a client app's `env/registry.ts` indexes `import.meta.env[key]` directly for exactly this reason (VL #13).
