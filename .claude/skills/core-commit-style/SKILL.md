---
name: core-commit-style
description: Conventional Commits, story-based commit splitting, resolving the PR base via script, and the squash-only merge rule. Use when committing, staging, creating a PR, or merging a pull request.
---
...
## Choosing the PR base — never guess (mandatory)

**Run `.claude/scripts/pr-base.sh` and pass its output to `--base`. Do not infer the base from anything else.**

```
gh pr create --base "$(.claude/scripts/pr-base.sh)" --title "…" --body "…"
```

`--verbose` prints the resolved base and the resulting diffstat on stderr — check it looks like your change before opening the PR.

**This repo integrates on `main`**, which is also the GitHub default branch, so the harness's `Main branch (you will usually use this for PRs): main` hint is correct here. The script still earns its keep: it records each worktree branch's fork point in git config (`branch.<name>.wtBase`) so the app diffs against the right base and the PR targets it reliably — rather than leaving either to inference.

Two traps the resolver handles for you, both of which have caused a wrong call:

- **`git push -u` destroys the upstream record.** It rewrites `@{u}` from the base to `origin/<own-branch>`. Anything reading upstream to find the base breaks the moment you push. `branch.<name>.wtBase` exists precisely because nothing else writes it.
- **The reflog records a SHA, not a branch.** `git worktree add` writes `Created from <sha>`. Naive prefix-stripping leaves a 40-char hex string that matches no ref, silently yielding no base at all. The resolver maps it back to a branch.

**Symptom you must recognise:** a PR diff far larger than your change (hundreds of files, deletions in areas you never touched) means the base is wrong. Stop and re-resolve. Do not open it.
...
**This one is enforced mechanically.** `.claude/hooks/check-pr-title-casing.sh` is a `PreToolUse(Bash)` guard that inspects `gh pr create` / `gh pr edit` titles and **blocks the call** (exit 2) on a missing/unknown type, a capitalized summary, or past tense — returning a corrected title. If you get blocked, take the correction; don't work around the hook by editing the title through another path.
