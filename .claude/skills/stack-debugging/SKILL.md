...
description: Reading runtime values without asking a human to relay them: the isolated headless browser for self-tests, and the LAN dev log sink any language can post to. Use when debugging a device or PWA bug, or when you need logs you cannot see.
---

# Debugging & device telemetry

**Principle: never make the human relay logs by hand.** When a bug needs runtime values — especially on a real device — instrument the code to ship logs somewhere *you* can read, then reproduce and read them yourself. Hand-typed values are slow, lossy, and a last resort. (Pairs with `platform-ios-webkit` → "trust the device over the simulator".)
